...
def _solve_maxbreach(
    ...
    """
    free = [(i, j) for (i, j) in graph.arcs if not (i,j) in interdicted]
...
def _solve_minbreachpath(
    ...
    for attack in attacks:
        ...
        sr=0
        for idx in attack:
            (path,rew)=all_paths[idx]
            sr+=rew
            elem.append(rew*(1.-u[idx]))
        m.addConstr(eta >= sum(elem))

        
    # Constraint (8c): u_p <= sum_{(i,j) in path_p} x[i,j]
    for idx, (path, _) in enumerate(all_paths):
        m.addConstr(
            u[idx] <= gp.quicksum(x[i, j] for (i, j) in path),
    ...
    lb = m.ObjVal
    interdict = [(i, j) for (i, j) in all_arcs if x[i, j].X>0.5]
    m.dispose()
    return lb, interdict
...
def run_paper_algorithm(
    ...
    while time.time()-t0<7200:
        ...
        for path in paths:
            ...
            if (path, rew) not in all_paths:
                goal_node = path[-1][1]
                rew = graph.nodes[goal_node].reward
                all_paths.append((path, rew))
            attack.append(all_paths.index((path, rew)))
